...
是同一件商品。身分由 **型號 + 規格** 決定，品牌與顏色都不是必要條件：

| level | 依據 | 說明 |
|---|---|---|
| `strong` | 獨特型號（≥5 碼英數混合）+ 品牌 | 如 `WH-1000XM5` |
| `medium` | 獨特型號（無品牌），或品牌 + 短型號 | 如 PChome 沒寫品牌的標題 |
| `loose` | 品牌 + 英數詞（AirPods 這類無型號商品） | 需抽查 |
| `weak` | 標題詞集雜湊 | 幾乎只能追蹤同平台歷史價 |

兩條刻意的規則：

**顏色不列入身分。** 同型號不同顏色算同一件商品。原本顏色是指紋的一部分，
但實測真實標題後發現這讓跨平台比對大量失敗 —— momo 寫
`【SONY 索尼】WH-1000XM5 …` 不帶顏色，PChome 寫 `WH-1000XM5 黑色 …` 帶顏色，
同一件商品因此拆成兩張各自只有一個通路的卡片。顏色仍會抽出來供顯示。

**容量與數量列入身分。** DDR5 32GB 與 16GB 是不同商品，兩入裝與單入也是。
差別在於它直接影響價格，顏色不會。

**型號要夠獨特才能單獨當合併依據**（≥5 碼、英數混合）。`DDR5` 只有 4 碼，
是規格標準不是型號 —— 放行會把 Kingston 與 Crucial 的記憶體併成同一件，
而且兩者價格接近，價差防線也攔不住。這種靜默的錯誤合併最危險。

改指紋規則時記得把 `config.FP_VERSION` 加一：舊鍵的商品不會再被採集碰到，
卻仍留在 catalog 裡，網站會永遠顯示一批不再更新的舊價格。加一之後採集前會
自動清空重建。

**遇到對不上的真實案例，請進 `tests/test_normalize.py` 補一組測資。** 目前已固化的坑：

- momo 的 `【SONY 索尼】…` 格式 —— 品牌藏在會被清理掉的括號裡
- PChome 標題常常完全沒有品牌字樣 —— 品牌放在獨立欄位
- `DDR5`、`XM5` —— 字母結尾接數字的型號原本抓不到
- `AirPods Pro 2` —— 完全沒有可辨識型號
- 同型號不同容量不可合併；同型號不同顏色必須合併
